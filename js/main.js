...
const hiddenSlider = document.querySelector('.benefits__slider');
...
burgerBtn.addEventListener('click', function () {
  ...
  hiddenSlider.classList.toggle('hidden');
...
//ACCORDION
document.querySelectorAll('.accordion__item').forEach((accordion, index) => {
  const header = accordion.querySelector('.accordion__btn');
  const content = accordion.querySelector('.accordion__answer');
  accordion.addEventListener('click', () => {
    const openItem = document.querySelector('.accordion__item.active');
    if (openItem && openItem !== accordion) {
      openItem.classList.remove('active');
      const openContent = openItem.querySelector('.accordion__answer');
      if (openContent) openContent.style.maxHeight = null;
      const openBtn = openItem.querySelector('.accordion__btn');
...
window.addEventListener('DOMContentLoaded', () => {
  const accIndex = localStorage.getItem('openAccordion');
  document.querySelectorAll('.accordion__item').forEach((accordion, index) => {
    const header = accordion.querySelector('.accordion__btn');
    const content = accordion.querySelector('.accordion__answer');
    if (index === 0) {
...
// SWIPER BENEFITS
const benefitsSlider = document.querySelector('.benefits-swiper');
const benefitsWrapper = document.querySelector('.benefits__boxSlider');
...
function toggleSwiper() {
  if (window.innerWidth <= 650) {
    if (!swiperInstance) {
      ...
      benefitsWrapper.classList.add('swiper-wrapper');
      benefitsWrapper.classList.remove('benefits__box');
      benefitsSlides.forEach(slide => {
        slide.classList.add('swiper-slide');
      });
      console.log('swiper on');
      swiperInstance = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"]('.benefits-swiper', {
        slidesPerView: 1,
        spaceBetween: 20,
        allowTouchMove: true,
        simulateTouch: true,
    ...
    if (swiperInstance) {
      benefitsSlider.classList.remove('swiper');
      benefitsWrapper.classList.remove('swiper-wrapper');
      benefitsWrapper.classList.add('benefits__box');
      benefitsSlides.forEach(slide => {
...
document.querySelectorAll('.review-card__more').forEach(button => {
  button.addEventListener('click', () => {
    ...
    const text = reviewCard.querySelector('.review-card__text');
    const collapsedHeight = window.innerWidth <= 610 ? 148 : 162;
    // if (text.scrollHeight <= collapsedHeight + 5) {
    //   alert('ОТЗЫВ ПОЛНОСТЬЮ ОТОБРАЖАЕТСЯ');
    //   return;
    // }

    if (text.classList.contains('expanded')) {
      // Скрыть текст
      text.style.maxHeight = text.scrollHeight + 'px'; // сначала зафиксировать текущую высоту
      requestAnimationFrame(() => {
      ...
      // Показать текст
      text.style.maxHeight = text.scrollHeight + 'px'; // раскрыть до полной высоты
...
selects.forEach(select => {
  ...
  const selectedText = select.querySelector('.select__selected span');
  const optionsBox = select.querySelector('.select__options');
...
//TG CHAT CONNECTION
const TOKEN = "8493422598:AAGzBDX9Zj3JNwthkQjjRsUwuMrYGp4kQAU";
const CHAT_ID = "7189669377";
const URL = `https://api.telegram.org/bot${TOKEN}/sendMessage`;
const forms = document.querySelectorAll('.form');
forms.forEach(form => {
  const options = form.querySelectorAll('.select__option');
  const selectedText = form.querySelector('.form__textarea');
  form.addEventListener('submit', e => {
    const checkbox = form.querySelector("#ch1");
    if (!checkbox.checked) {
    ...
    const messenger = selectedText.textContent.trim();
...
📝 Комментарий: ${comment}
    ...
    fetch(URL, {
      ...
      if (data.ok) {
        ...
        form.reset();
        if (selectedText) selectedText.textContent = "Telegram"; // сброс select
